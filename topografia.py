...
class TopografiaPlugin:
    ...
    def initGui(self):
        # Configurar rutas de iconos (ajustado para estructura con subdirectorio tools/)
        icon_dir = os.path.join(self.plugin_dir, 'icons')
        self.icon_paths = {
            'about': os.path.join(icon_dir, 'about.png'),
            'icon': os.path.join(icon_dir, 'icon.png'),
            'lineas': os.path.join(icon_dir, 'lineas.png'),
            'poligonos': os.path.join(icon_dir, 'poligonos.png')
        }

        # Cargar módulos desde el subdirectorio tools/
        tools_dir = os.path.join(self.plugin_dir, 'tools')
        
        self.poligonos_module = import_module_from_path(
            os.path.join(tools_dir, 'poligonos.py'), 
            'poligonos'
        )
        
        self.lineas_module = import_module_from_path(
            os.path.join(tools_dir, 'lineas.py'),
            'lineas'
        )
        
        self.about_module = import_module_from_path(
            os.path.join(tools_dir, 'about.py'),
            'about'
        )
        ...
        # Añadir a la barra de herramientas (excepto About)
